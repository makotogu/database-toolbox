...
public class GaussDbDialect implements DatabaseDialect {
    ...
    public String upsertSql(String tableName, List<UpsertColumn> targetColumns, List<String> matchKeys) {
        return batchUpsertSql(tableName, targetColumns, matchKeys, 1);
    }

    public String batchUpsertSql(String tableName,
                                 List<UpsertColumn> targetColumns,
                                 List<String> matchKeys,
                                 int rowCount) {
        int rows = rowCount <= 0 ? 1 : rowCount;
        List<String> rowSelects = new ArrayList<String>();
        for (int i = 0; i < rows; i++) {
            rowSelects.add(sourceRowSql(targetColumns));
        }
        return mergeSql(tableName, targetColumns, matchKeys, join(rowSelects, " UNION ALL "));
    }

    public String mergeFromTableSql(String targetTable,
                                    String sourceTableSql,
                                    List<UpsertColumn> targetColumns,
                                    List<String> matchKeys) {
        return mergeSql(targetTable, targetColumns, matchKeys, sourceTableSql);
    }

    private String sourceRowSql(List<UpsertColumn> targetColumns) {
        List<String> sourceColumns = new ArrayList<String>();
        for (UpsertColumn targetColumn : targetColumns) {
            String quoted = quoteIdentifier(targetColumn.getName());
            sourceColumns.add(parameterExpression(targetColumn) + " AS " + quoted);
        }
        return "SELECT " + join(sourceColumns) + " FROM dual";
    }

    private String mergeSql(String tableName,
                            List<UpsertColumn> targetColumns,
                            List<String> matchKeys,
                            String sourceSql) {
