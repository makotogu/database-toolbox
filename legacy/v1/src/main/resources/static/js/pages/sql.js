                ...
                <div class="panel-body">
                    <form id="sql-form" class="form">
                        <div class="form-section">
                            <div class="section-title">执行范围</div>
                            <div class="grid-3">
                        ...
                        <div class="notice warn">写操作会先进入确认流程；确认后才会真正提交到目标数据源。</div>
...
function renderResult(result) {
    ...
    if (result.confirmationRequired) {
        return '<div class="danger-zone">检测到写操作，请确认后再执行。<pre>' + escapeHtml(lastSqlPayload ? lastSqlPayload.sql : '') + '</pre></div>';
    }
    ...
    return '<div class="sql-result-table">' + table(columns, result.rows || [], { empty: '查询无结果' }) + '</div>';
}
