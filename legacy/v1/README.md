...
目标调整为类似 DBeaver 的本地数据库工作台：用户选择 JDBC 驱动，浏览表结构与内容，执行 SQL、存储过程、代码块和多语句脚本，并查看 EXPLAIN。继续保持 Java 8 和单 JAR 启动。

- [重构方案与架构](notes/reconstruction/README.md)
- [分阶段实施与验收清单](notes/reconstruction/ACCEPTANCE.md)
- [现状证据与构建验证](notes/reconstruction/BASELINE.md)
