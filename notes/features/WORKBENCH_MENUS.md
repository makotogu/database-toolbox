...
- “查看结构”仅取元数据，不自动读取表数据。“查看数据”沿用既有表预览路径。
- 单元格菜单沿用只读条件与安全写回路径。菜单绑定原执行、结果行数组及 DOM 单元格，刷新后失效；普通查询结果不能通过菜单写回。
...
- 最终 JDK 8 `mvn -o -B -ntp clean verify`：111 项，110 通过、1 跳过（未配置真实 PostgreSQL 集成环境），0 失败/错误。`node --check app.js`、ES 模块语法检查、`sh -n scripts/database-toolbox.sh`、既有 Node 异步/SQL/连接字段回归及 `git diff --check` 通过。
- 最终 JAR 的 9 个静态资源逐文件与源码一致；不使用静态资源覆盖，临时存储下通过全部 `smoke-menus-ui.cjs` 浏览器检查，并人工查看桌面/窄屏截图。`verify-release.py` 的 9 项独立检查通过，覆盖空目录启动、静态首页与资源、默认驱动查询、请求保护、目录锁、重启配置及 SQL 草稿恢复不执行。
- 独立功能包：`dist/features/workbench-menus/`（忽略提交）。JAR SHA-256：`4c6f19fd76619dde0bd58f1f4961723dbe668cca393fa7b2a91885a548d161ad`。
