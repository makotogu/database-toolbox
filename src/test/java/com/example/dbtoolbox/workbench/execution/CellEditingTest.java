...
class CellEditingTest {
    ...
    @Test void legacyPostgresMetadataWithoutGeneratedFlagCanEditOrdinaryCells() throws Exception {
        create();legacyMetadata("PostgreSQL", false);
        ExecutionRecord source=preview("CELLS");assertTrue(grid(source).columns.get(1).editable,grid(source).readOnlyReason);
        assertEquals("SUCCEEDED",execute(edit(source,0,1,"compatible",false)).state);
        assertEquals("compatible",value("SELECT name FROM cells WHERE id=1"));
    }
    @Test void gaussCatalogKeepsGeneratedAndIdentityColumnsReadOnly() throws Exception {
        sql("CREATE TABLE cells(id INT PRIMARY KEY, name VARCHAR, derived VARCHAR GENERATED ALWAYS AS (UPPER(name)), seq BIGINT GENERATED BY DEFAULT AS IDENTITY)");
        sql("INSERT INTO cells(id,name) VALUES(1,'old')");legacyMetadata("GaussDB", false);
        ExecutionRecord source=preview("CELLS");assertTrue(grid(source).columns.get(1).editable,grid(source).readOnlyReason);
        assertFalse(grid(source).columns.get(0).editable);assertFalse(grid(source).columns.get(2).editable);assertFalse(grid(source).columns.get(3).editable);
    ...
    @Test void missingCatalogGenerationFlagOnlyAllowsColumnsWithoutDefaults() throws Exception {
        ...
        assertFalse(grid(source).columns.get(3).editable);assertTrue(grid(source).columns.get(3).readOnlyReason.contains("无法确认"));
        assertEquals("SUCCEEDED",execute(edit(source,0,1,"safe",false)).state);
    }
    @Test void missingIdentityFlagRequiresIndependentEvidence() throws Exception {
        create();LegacyMetadata fixture=legacyMetadata("PostgreSQL",false);fixture.missingIdentity=true;
        ExecutionRecord source=preview("CELLS");assertFalse(grid(source).columns.get(1).editable);
        fixture.identityColumn=true;source=preview("CELLS");assertTrue(grid(source).columns.get(1).editable);
        assertFalse(grid(source).columns.get(3).editable); // Could be a sequence default; never guess from an expression.
        assertEquals("SUCCEEDED",execute(edit(source,0,1,"safe",false)).state);
    }
    @Test void unknownFlagsOnOtherDriversHaveSpecificReadOnlyReason() throws Exception {
        create();legacyMetadata("H2",false);ExecutionRecord source=preview("CELLS");
        assertNull(grid(source).readOnlyReason);assertFalse(grid(source).columns.get(1).editable);
        assertTrue(grid(source).columns.get(1).readOnlyReason.contains("元数据不完整"));
    }
    @Test void catalogFailurePreservesManualTransactionAndRedactsVendorMessage() throws Exception {
        create();sessions.transaction(session.id,"AUTO_COMMIT",false);sql("UPDATE cells SET name='prior' WHERE id=2");
        LegacyMetadata fixture=legacyMetadata("GaussDB",true);ExecutionRecord source=preview("CELLS");
        assertTrue(grid(source).readOnlyReason.contains("生成列元数据兼容查询失败"));
        assertTrue(grid(source).readOnlyReason.contains("42501"));assertFalse(grid(source).readOnlyReason.contains("sensitive"));
        assertEquals(1,fixture.savepointRollbacks);assertEquals("second",value("SELECT name FROM cells WHERE id=2"));
        fixture.catalogFailure=false;
        assertEquals("SUCCEEDED",execute(edit(preview("CELLS"),0,1,"pending",false)).state);
        assertEquals("old",value("SELECT name FROM cells WHERE id=1"));
        sessions.transaction(session.id,"COMMIT",false);
        assertEquals("prior",value("SELECT name FROM cells WHERE id=2"));assertEquals("pending",value("SELECT name FROM cells WHERE id=1"));
    }
    @Test void catalogIsRecheckedBeforeWriting() throws Exception {
        create();LegacyMetadata fixture=legacyMetadata("GaussDB",false);ExecutionRecord source=preview("CELLS");
        assertTrue(grid(source).columns.get(3).editable);
        ExecutionRequest request=edit(source,0,3,"8",false);request.confirmationToken=executions.prepare(request).confirmationToken;
        fixture.generationColumn=false;ExecutionRecord result=executions.submit(request);await(result);
    ...
    private LegacyMetadata legacyMetadata(String product, boolean catalogFailure) throws Exception {
        LegacyMetadata fixture=new LegacyMetadata();fixture.catalogFailure=catalogFailure;
        Connection raw=session.connection;DatabaseMetaData original=raw.getMetaData();
        DatabaseMetaData md=proxy(DatabaseMetaData.class,(method,args)->{
            if(method.getName().equals("getDatabaseProductName"))return product;
            Object result=invoke(original,method,args);
            if(method.getName().equals("getColumns"))return proxy(ResultSet.class,(m,a)->{
                if(m.getName().equals("getString")&&("IS_GENERATEDCOLUMN".equals(a[0])||fixture.missingIdentity&&"IS_AUTOINCREMENT".equals(a[0])))throw new SQLException("Missing optional label", "42703");
                if(m.getName().equals("getMetaData")) {
                    ResultSetMetaData meta=((ResultSet)result).getMetaData();
                    return proxy(ResultSetMetaData.class,(mm,aa)-> {
                        Object v=invoke(meta,mm,aa);
                        return (mm.getName().equals("getColumnLabel")||mm.getName().equals("getColumnName"))&&("IS_GENERATEDCOLUMN".equals(v)||fixture.missingIdentity&&"IS_AUTOINCREMENT".equals(v))?"OMITTED_FLAG":v;
                    });
                }
                return invoke(result,m,a);
            });
            return result;
        });
        session.connection=proxy(Connection.class,(method,args)->{
            if(method.getName().equals("getMetaData"))return md;
            if(method.getName().equals("rollback")&&args!=null&&args.length==1)fixture.savepointRollbacks++;
            if(method.getName().equals("prepareStatement")&&((String)args[0]).startsWith("SELECT a.*, d.* FROM pg_catalog.pg_attribute")) {
                if(fixture.catalogFailure)throw new SQLException("sensitive vendor detail", "42501");
