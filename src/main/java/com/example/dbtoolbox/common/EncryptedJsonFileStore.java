...
public class EncryptedJsonFileStore<T> {
    ...
    private T decode(byte[] bytes) throws Exception {
        ...
        Cipher cipher;
        if (modern) {
            if (bytes.length < HEADER.length + 12 + 16 || bytes[4] != HEADER[4]) {
                throw new IOException("配置格式无效");
            }
            cipher = Cipher.getInstance("AES/GCM/NoPadding");
            cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(loadKey(false), "AES"),
                    new GCMParameterSpec(128, Arrays.copyOfRange(bytes, 5, 17)));
            cipher.updateAAD(HEADER);
            bytes = Arrays.copyOfRange(bytes, 17, bytes.length);
        } else {
            if (bytes.length == 0 || bytes.length % 16 != 0) {
                throw new IOException("旧配置格式无效");
            }
    ...
    public synchronized void upgradeEncryption() {
        try {
            ...
            throw new AppException("升级本地配置加密失败，原配置或备份已保留；请检查权限与备份");
        }
    }
    ...
    public synchronized void write(T value) {
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(loadKey(!Files.exists(file)), "AES"),
                    new GCMParameterSpec(128, iv));
            cipher.updateAAD(HEADER);
            byte[] ciphertext = cipher.doFinal(json);
            byte[] bytes = new byte[17 + ciphertext.length];
            System.arraycopy(HEADER, 0, bytes, 0, HEADER.length);
            System.arraycopy(iv, 0, bytes, 5, 12);
            System.arraycopy(ciphertext, 0, bytes, 17, ciphertext.length);
            PrivateFiles.replace(file, bytes);
        } catch (Exception ex) {
            throw new AppException("写入本地配置失败：请检查密钥、目录权限和磁盘空间；未清空配置");
        }
    }
    ...
    private byte[] loadKey(boolean create) throws IOException {
        ...
        if (!Files.exists(keyFile, LinkOption.NOFOLLOW_LINKS)) {
            ...
            try {
                PrivateFiles.writeNew(keyFile, Base64.getEncoder().encode(key));
            } catch (FileAlreadyExistsException ignored) {
                // Another store created the shared key first.
            }
        }
