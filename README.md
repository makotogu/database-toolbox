...
品牌、连接选择和“文件 / 执行 / 视图 / 帮助”共用一条顶部栏。菜单集中提供 SQL 文件、新建连接、执行范围、事务、对象刷新和快捷键入口。不可用的操作会显示原因；菜单不会绕过执行确认。桌面也可在“视图”中收起数据库导航，留出更多编辑空间。
...
- 拖动导航右边缘、编辑器与结果区之间的分隔线可调整面板大小；也可聚焦分隔线后用方向键微调，双击恢复该面板默认大小。“视图 → 恢复默认布局”可同时重置。浏览器只记住宽度和比例，不在布局设置中保存 SQL 或连接信息；窗口变窄时会收起导航。
- 拖动标签可调整顺序，右键菜单也提供向左/向右移动。排序保留当前标签、SQL 选区、结果与数据库会话；“关闭右侧标签”按调整后的顺序执行。布局设置不保存标签顺序；已启用 SQL 草稿的连接仍按草稿机制保存 SQL 标签顺序。

这些功能需构建当前源码，未改动公开 Release。验证记录见 [工作台菜单](notes/features/WORKBENCH_MENUS.md) 与 [单层顶部栏与拖拽布局](notes/features/UNIFIED_LAYOUT.md)。
