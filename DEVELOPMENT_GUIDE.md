...
`menus.js` 只管理一个共享弹出菜单、定位、键盘和焦点；标签/连接/结果上下文与异步错误由 `app.js` 管理。菜单文字使用 `textContent`。菜单项打开时提供禁用原因，执行时再次检查原目标是否存在及其忙状态。表结构入口仅请求元数据，不隐式执行表预览。复制 SQL 标签不能复制会话、执行记录或服务端编辑快照。
